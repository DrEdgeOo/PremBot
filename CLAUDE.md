...
## Remote access & security (non-negotiable)

PremBot can be driven remotely (an external agent calling an MCP server
hosted in the CEP helper). The moment that surface is tunneled, PremBot
becomes an internet-reachable program that edits files and drives an
application on this machine. These are not preferences — getting them
wrong is the one failure in this project that cannot be fixed by
iterating. Treat every rule below as load-bearing.

**Two surfaces, never merged**
- The UXP↔helper channel (`127.0.0.1:53210`) is local-only. It must stay
  bound to `127.0.0.1` and must never be tunneled.
- The MCP surface runs on its own **separate port**. Only that port is
  ever exposed.

**Authentication**
- Every request to the MCP surface requires a bearer token in a header.
  Reject anything without it *before* parsing the body. Compare with a
  constant-time comparison, not `===`.
- The token lives in an env var or a local config file — never in the
  repo, never in a tool description, never in a log line.
- No token, no service: if the token is unset at startup, the MCP server
  must refuse to start rather than run open.

**CORS**
- `send()` in `client/js/bridge.js` sets `Access-Control-Allow-Origin: *`.
  That is fine for a loopback-bound helper and unacceptable on an exposed
  surface. The MCP responder must NOT reuse `send()` as-is; do not send a
  wildcard origin on the MCP port.

**Tool exposure**
- Remote callers get an explicit **allowlist**, not the whole registry.
  Default to deny; add tools deliberately.
- **Never expose a generic "run this script" tool remotely.**
  `qeDomFunction` and any future arbitrary-ExtendScript path are
...
- Any path argument must resolve inside an allowlisted root (the project
  media folder, the `%TEMP%\PremBot-*` caches,
  `<Documents>\PremBot LUTs\`). Reject absolute paths outside those roots
  and reject `..` traversal. This applies to Python sidecar args too —
  they reach `child_process`.

**Operational**
- Log every remote call: timestamp, tool, arguments, result status. Never
  log base64 payloads or the token.
- The remote-control toggle in the UXP panel defaults to **OFF** and shows
...
**When in doubt**
Prefer the local transport. Interactive local use needs none of this
exposure. The tunnel exists to enable browser-based work, not because it
